...
const Search = ({ searchText: searchedText }) => {
  const { data, isLoading } = useInfiniteScroll({
    url: `/api/tmdb/search/${decodeURI(searchedText)}?includeAdult=${true}`,
  });
    ...
    <div className={styles.search}>
      <div className={styles.searchInputContainer}>
        <form onSubmit={handleSearch} className={styles.searchInput}>
          <input
            autoFocus={pathname.split('/').at(-1) === 'search' ? true : false}
            placeholder='Search...'
            type='text'
            value={searchText}
      ...
      <main className='resultContainer'>
        {!data ||
          (data?.length === 0 && (
            <div className={styles.emptyMessage}>{isLoading ? 'Searching ... ' : 'Oops! I am empty.'}</div>
          ))}
                  ...
                  <Link href={`/${media_type}/${id}`} key={id} className={styles.card}>
                    <div className={styles.left}>
                      <div className={styles.topLeft}>
                        <Image
                          src={`${tmdbImageBaseUrl}/w92/${poster_path}`}
                          alt={`${name || title} photo`}
                          width={92}
                          height={138}
                        />
                        <div>
                          ...
                          <p className={styles.releaseDate}>
                            {release_date?.split('-')[0] || first_air_date?.split('-')[0]}, {media_type}
                          </p>
                    ...
                    <div className={styles.right}>
                      {backdrop_path && (
                        <Image
                          src={`${tmdbImageBaseUrl}/w300/${backdrop_path}`}
                          alt={`${name || title} photo`}
                          width={300}
                          height={169}
                        />
