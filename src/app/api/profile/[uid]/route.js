...
export const POST = async (request) => {
    try {
        ...
        // validation part 
        if (!name && (!pin && hasPin)) {
            return new NextResponse(JSON.stringify({ message: "Check the box below or create PIN", fields: ["name", "pin"] }), { status: 401 })
        }

        if (!name) {
            return new NextResponse(JSON.stringify({ message: "Name is required", fields: ["name"] }), { status: 401 })
        }
        if (!pin && hasPin) {
            return new NextResponse(JSON.stringify({ message: "Click on no PIN below or enter PIN", fields: ["pin"] }), { status: 401 })
        }
        ...
        if (allProfiles.length >= 3) {
            return new NextResponse(JSON.stringify({ message: "Maximum profile limit reached", fields: ["name"] }), { status: 401 })
        }
        const existingProfile = allProfiles.filter((profile) => profile.name == name)
        if (existingProfile.length > 0) {
            return new NextResponse(JSON.stringify({ message: "Enter another name", fields: ["name"] }), { status: 401 })
        }
        ...
        return new NextResponse(JSON.stringify({ id: savedProfile._id, uid: savedProfile.uid, name: savedProfile.name, isKid: savedProfile.isKid, avatar: savedProfile.avatar, hasPin: savedProfile.hasPin }))
