...
export const POST = async (request) => {
  try {
    ...
    if (!pin) {
      return new NextResponse(JSON.stringify({ message: 'Pin is required', fields: ['pin'] }), {
        status: 401,
      });
    }
    ...
    if (profile === null) {
      return new NextResponse(JSON.stringify({ message: 'No profile with that name', fields: ['name'] }), {
        status: 401,
      });
    }
