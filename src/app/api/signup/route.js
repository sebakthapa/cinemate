...
export const POST = async (request) => {
  try {
    ...
    if (!confirmPassword) {
      return new NextResponse(
        JSON.stringify({ message: 'Password Confirmation is required', fields: ['confirmPassword'] }),
        { status: 401 }
      );
    }
    ...
    if (confirmPassword !== password) {
      return new NextResponse(
        JSON.stringify({ message: `Confirm password did't match  required`, fields: ['confirmPassword'] }),
        { status: 401 }
      );
    }
    ...
    if (existingUser) {
      return new NextResponse(JSON.stringify({ message: 'Email is already registered', fields: ['email'] }), {
        status: 401,
      });
    }
